...
DEFAULT_SETTINGS = {
    ...
    "addonBeepDirectionA": LEFT,  # Changed from CENTER to LEFT
    "addonBeepDirectionB": RIGHT,  # Changed from CENTER to RIGHT
    "isActive": True,
    "waveformType": 0,
...
class SoundAlignSettingsPanel(gui.settingsDialogs.SettingsPanel):
    ...
    def makeSettings(self, settingsSizer):
        ...
        waveform_choices = [_(name) for name in WAVEFORM_NAMES.values()]
        ...
        # New settings for speech and time-based announcements
...
class GlobalPlugin(globalPluginHandler.GlobalPlugin):
    ...
    def safeBeep(self, hz, length, left=50, right=50, *args, **kwargs):
        ...
        if soundType == PROGRESS_INDICATOR:
            ...
            if self.settings.get("waveformType") == 3: # Original Tone Beep
                ...
                # First, try to get progress value from the object
                percent = self.sound_processor.get_progress_percent(obj) if self.sound_processor else None
                
                # If object value is not valid, fall back to Hz-based calculation
                if not isinstance(percent, (int, float)) or percent < 0:
                    ...
                    if self.settings.get("maxFrequency") > self.settings.get("minFrequency"):
                        ...
                        percent = 50  # Default to 50% if min and max freq are the same
                ...
                # If percent is not available, calculate from frequency
                if percent is None:
                ...
                # Only play if percent has changed
    ...
    def handleProgressAnnouncements(self, percent, obj):
        ...
        # Check if progress object has changed
        ...
        if mixed_mode:
            # Mixed mode: speech at speech interval, beep at beep interval
            if percent % speech_interval == 0 and percent != self.last_spoken_percent:
                ...
                # Just update the tracking, the beep will be played by the normal processing
    ...
    def getSoundType(self, hz, length):
        ...
        # NVDA's progress indicator sounds - use more specific criteria
    ...
    def testProgress(self, direction, waveform_type):
        ...
        if waveform_type == 3: # Original Tone Beep
            log.info("SoundAlign: Testing progress with original tone beep")
            for i in range(0, 101, 2):
                # Calculate frequency from percentage for testing
    ...
    @script(
        description=_("Open SoundAlign settings"),
        gesture="kb:NVDA+Windows+S"
    )
    def script_openSettings(self, gesture):
        gui.mainFrame.popupSettingsDialog(gui.settingsDialogs.NVDASettingsDialog, SoundAlignSettingsPanel)
